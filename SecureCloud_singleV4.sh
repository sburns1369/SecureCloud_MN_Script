...
echo
echo
echo -e ${GREEN}"Are you sure you want to continue the installation of a SecureCloud Masternode?"
...
echo -e "${GREEN}Configuring SecureCloud Node${CLEAR}"
...
echo "externalip=$(hostname -I | cut -f1 -d' '):9191" >> /home/securecloud1/.securecloud/securecloud.conf
...
echo
echo -e ${YELLOW}"Launching SCN Node"${CLEAR}
...
echo
echo -e ${BOLD}"Your SCN Node has Launched."${CLEAR}
echo

echo -e "${GREEN}You can check the status of your SCN Masternode with"${CLEAR}
echo -e "${YELLOW} securecloud-cli -datadir=/home/securecloud1/.securecloud masternode status"${CLEAR}
...
echo -e "${BOLD} Masternode - IP: $(hostname -I | cut -f1 -d' '):9191${CLEAR}"
fi
